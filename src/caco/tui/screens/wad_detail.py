...
class WadDetailScreen(Screen):
    ...
    def _load_details(self) -> None:
        ...
        if wad.get("custom_iwad") or wad.get("custom_sourceport") or wad.get("custom_args") or companions:
            content.mount(Static(""))
            ...
            for comp in companions:
                status = "" if comp["enabled"] else " [dim](off)[/dim]"
                content.mount(self._make_row("File", f"{comp['filename']}{status}"))
