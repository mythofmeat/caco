...
fn rejects_sandbox_equal_to_fake_caco_home() {
    ...
    temp_env::with_var(
        "XDG_DATA_HOME",
        Some(fake.path().to_str().unwrap()),
        || {
            let err =
                SandboxPaths::new(caco_home.clone(), fake.path().to_path_buf()).unwrap_err();
...
fn rejects_sandbox_inside_fake_caco_home() {
    ...
    temp_env::with_var(
        ...
        || {
            let err =
                SandboxPaths::new(nested.clone(), fake.path().to_path_buf()).unwrap_err();
            assert!(matches!(err, CacoMcpError::SandboxPathUnsafe { .. }));
        },
    );
}
...
fn rejects_sandbox_equal_to_env_caco_home() {
    ...
    temp_env::with_vars(
        [
            ("XDG_DATA_HOME", Some("/nonexistent/xdg-sandbox-safety-tests")),
            ("CACO_HOME", Some(fake.path().to_str().unwrap())),
        ],
        || {
            let err =
                SandboxPaths::new(fake.path().to_path_buf(), fake.path().to_path_buf())
                    .unwrap_err();
            assert!(matches!(err, CacoMcpError::SandboxPathUnsafe { .. }));
        },
    );
