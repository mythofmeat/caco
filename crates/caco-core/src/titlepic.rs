...
/// Standard Doom PLAYPAL (first palette, 256 × RGB = 768 bytes).
/// Extracted from the Ultimate Doom v1.9ud IWAD.  PWADs almost never ship
/// their own PLAYPAL, so this fallback covers the vast majority of cases.
#[rustfmt::skip]
static DOOM_PALETTE: [u8; 768] = [
...
fn decode_doom_patch(data: &[u8], palette: Option<&[u8]>) -> Option<TitlepicImage> {
    ...
    let pal = palette.unwrap_or(&DOOM_PALETTE);
...
mod tests {
    ...
    #[test]
    fn test_extract_titlepic_no_palette_uses_doom_palette() {
        ...
        assert_eq!(img.width, 1);
        assert_eq!(img.height, 1);
        // Doom palette index 128 → first entry of the tan/brown ramp
        assert_eq!(img.pixels[0], DOOM_PALETTE[128 * 3]);
        assert_eq!(img.pixels[1], DOOM_PALETTE[128 * 3 + 1]);
        assert_eq!(img.pixels[2], DOOM_PALETTE[128 * 3 + 2]);
